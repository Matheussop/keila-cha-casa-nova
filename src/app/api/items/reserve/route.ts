...
import { reserveGiftItem } from "@/lib/data";
...
export async function POST(request: Request) {
  ...
  try {
    const item = await reserveGiftItem({
      itemId,
    ...
    const message = error instanceof Error ? error.message : "Nao foi possivel reservar o item.";
    const status = message.includes("configure") ? 503 : message.includes("reservado") ? 409 : 400;
